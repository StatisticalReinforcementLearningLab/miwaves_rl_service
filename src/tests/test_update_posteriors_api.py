...
class TestUpdatePosteriorsAPI(BaseTestCase):
    
    @mock.patch('src.server.DecisionTimeEndAPI.requests.post')
    def test_update_posteriors(self, mock_post):
        """ Test for updating posteriors """
        with self.client:
            ...
            self.assertTrue(action_data['act_prob'])
            self.assertTrue(action_data['action'] in [0, 1])
            self.assertTrue(action_data['decision_index'] == 1)
            self.assertTrue(action_data['policy_id'])
            self.assertTrue(action_data['rid'])
...
# class TestUpdatePosteriorsAPI_Test2(BaseTestCase):

#     @mock.patch('src.server.DecisionTimeEndAPI.requests.post')
#     def test_update_posteriors_after_one_timestep(self, mock_post):
#         """ Test for updating posteriors after only one timestep (not enough data)"""
#         with self.client:
#             response_client = register_client(self, 'joe@gmail.com', '123456')
#             data_register = json.loads(response_client.data.decode())
#             self.assertTrue(data_register['status'] == 'success')
...
#             self.assertTrue(data['message'] == 'Error while updating posteriors')